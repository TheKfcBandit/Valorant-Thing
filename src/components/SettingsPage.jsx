...
export default function SettingsPage({
  oauthSession, valorantConnected, onOAuthSignin, onOAuthSignout, player,
    ...
    <motion.div initial="hidden" animate="show" variants={{ hidden: {}, show: { transition: { staggerChildren: noAnim() ? 0 : 0.04 } } }} className="flex-1 flex flex-col min-h-0 p-5 gap-3 overflow-y-auto">
      ...
      <motion.div variants={{ hidden: { opacity: 0, y: 8 }, show: { opacity: 1, y: 0 } }} transition={noAnim() ? T0 : { duration: 0.2 }} className="rounded-xl bg-base-700 border border-border divide-y divide-border">
        <div className="px-4 pt-3 pb-1">
          <h2 className="text-[10px] font-display font-bold text-text-muted uppercase tracking-wider">Riot Account (offline mode)</h2>
        </div>
        <div className="p-4 space-y-3">
          {valorantConnected && !oauthSession ? (
            ...
            <>
              <p className="text-xs font-body text-text-muted">
                Signed in as <span className="text-text-primary font-display">{player?.game_name}#{player?.game_tag}</span>. Live data without Valorant running. Session lasts ~60 min; sign in again to refresh.
              </p>
              <button
            ...
            <>
              <p className="text-xs font-body text-text-muted">
                Lets Store, Wrapped, Coach, and Spend tracker show live data when Valorant isn't running. Uses Riot's official login page in a popup — your password never touches this app.
              </p>
              <button
          ...
          {oauthError && (
            <p className="text-[11px] font-body text-val-red">{oauthError}</p>
          )}
        </div>
      </motion.div>
