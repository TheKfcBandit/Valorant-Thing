  ...
  useEffect(() => {
    ...
    const buildRpc = () => {
      ...
      if (status === "disconnected") {
        ...
        if (phase === "pregame") {
          details = lockedAgentNameRef.current
            ? `Locked ${lockedAgentNameRef.current}`
            : "Agent Select";
          ...
          if (
            ...
            if (agent && agent.uuid === "none") {
              ...
              if (mode === "instant") {
                ...
                pendingLockRef.current.set(matchId, {
                  agentId: agent.uuid,
                  agentName: agent.displayName,
                });
                lockedAgentNameRef.current = agent.displayName;
      ...
      >
                            ...
                            {updateInfo.release_notes.split("\n").map((line, i) => {
                              if (line.startsWith("### "))
                                return (
                                  <p
                                    key={i}
                                    className="text-text-secondary font-semibold text-[11px] pt-1.5"
                                  >
                                      ...
                                      {rel.notes.split("\n").map((line, i) => {
                                        if (line.startsWith("### "))
                                          return (
                                            <p
                                              key={i}
                                              className="text-text-secondary font-semibold text-[11px] pt-1"
                                            >
                                              {renderMarkdownInline(line.slice(4))}
                                            </p>
                                          );
                                        if (line.startsWith("## "))
                                          return (
                                            <p
                                              key={i}
                                              className="text-text-primary font-bold text-xs pt-1.5"
                                            >
                                              {renderMarkdownInline(line.slice(3))}
                                            </p>
                                          );
        ...
        <div className="flex flex-1 min-h-0">
          ...
          <main className="flex-1 flex min-h-0 relative">
              ...
              {activeTab === "instalock" && (
                ...
                >
                  <InstalockPage
                    ...
                    onConfigChange={(cfg) => {
                      instalockConfigRef.current = cfg;
                    }}
                    connected={status === "connected"}
                  />
                </motion.div>
            ...
            <div
              className={`absolute inset-0 flex min-h-0 ${activeTab === "fakestatus" ? "" : "hidden"}`}
            >
              <FakeStatusPage
                connected={status === "connected"}
